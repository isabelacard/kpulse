  ...
  try {
    if (!session_id) {
      ...
      if (!session_id) {
        return res
          .status(400)
          .json({ error: "No session_id found for this user." });
      }
    }
    ...
    if (motorScore >= 85) {
      ...
      color = "#28a745"; // Verde
      recommendation =
        "Tu control motor fino se encuentra en excelente estado.";
      ...
      recommendation =
        "Se recomienda continuar con ejercicios de rehabilitación motriz.";
    } else {
      classification = "Requiere atención";
      color = "#dc3545"; // Rojo
      recommendation =
        "Te recomendamos consultar con un especialista para un programa de rehabilitación activa.";
    }

    // Preparar respuestas de la encuesta para el email
    const surveyHtml = surveyResponses
      .map(
        (r) => `
            <tr>
    ...
    const sessionDate = sessionData?.started_at
      ? new Date(sessionData.started_at).toLocaleDateString()
